...
// Description
// Lambda@Edge function for modifying the origin request.
// Last modified
// 1 hour ago
// Function ARN
// arn:aws:lambda:us-east-1:178589013767:function:mdn-content-origin-request:1809

import { resolveFundamental } from "../components/redirects";
import { Request, Response, NextFunction } from "express";
import { decodePath, encodePath, slugToFolder } from "../../libs/slug-utils";
import { VALID_LOCALES } from "../components/constants";
import REDIRECTS from "../utils/build-redirects";
...
const originRequest = (
   ...
   */
  const requestURILowerCase = request.url.toLowerCase();
  const host = request.hostname.toLowerCase();
  const query_params = Object.entries(request.query)
    .map(([k, v]) => `${k}=${v}`)
    .join("&");
  const qs = query_params ? `?${query_params}` : "";
  ...
  if (request.path.startsWith("//")) {
    ...
    return response.redirect(`/${request.path.replace(/^\/+/g, "")}` + qs);
  }

  let { url, status } = resolveFundamental(request.url);
  if (url) {
    ...
    if (request.query) {
      url += (url.includes("?") ? "&" : "?") + request.query;
    }
    return redirect(url, {
      status,
  ...
  if (
    ...
    return response.redirect(`/${locale}${path || "/"}` + qs);
  }

  // At this point, the URI is guaranteed to start with a forward slash.
  const uriParts = request.path.split("/");
  ...
  if (LOCALE_URI_WITHOUT_TRAILING_SLASH.has(requestURILowerCase)) {
    ...
    response.setHeader("Cache-Control", `max-age=${THIRTY_DAYS},public`);
    return response.redirect(request.path + "/" + qs);
    ...
    //For server rendering of docs we can ignore this
    ...
    response.setHeader("Cache-Control", `max-age=${THIRTY_DAYS},public`);
    return response.redirect(request.path.slice(0, -1) + qs);
  }
  ...
  const decodedUri = decodePath(request.path);
