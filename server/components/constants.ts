const VALID_LOCALES: Map<string, string> = new Map(
  ["en-US", "es", "fr", "ja", "ko", "pt-BR", "ru", "zh-CN", "zh-TW"].map(
    (x) => [x.toLowerCase(), x]
  )
);

const RETIRED_LOCALES: Map<string, string> = new Map(
  [
    "ar",
    "bg",
    "bn",
...
const LOCALE_ALIASES: Map<string, string> = new Map([
...
const PREFERRED_LOCALE_COOKIE_NAME: string = "preferredlocale";
const ACTIVE_LOCALES: Set<string> = new Set([
  "en-us",
  "es",
  "fr",
  "ja",
...
const CSP_SCRIPT_SRC_VALUES: string[] = [
  "'report-sample'",
  "'self'",
...
const CSP_DIRECTIVES: { [key: string]: string[] } = {
  "default-src": ["'self'"],
...
const cspToString = (csp: { [key: string]: string[] }) =>
  Object.entries(csp)
